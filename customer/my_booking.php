...
// Query untuk mengambil data booking (TAMBAHKAN qris_expiry)
$q = mysqli_query($conn, "SELECT b.*, s.nama_layanan, s.harga,
                                 ...
                                 TIMESTAMPDIFF(SECOND, NOW(), b.qris_expiry) as qris_seconds_left,
                                 b.qris_expiry as qris_expiry_time
                         FROM bookings b 
...
<head>
    ...
    <style>
        ...
        .stats-payment { background: linear-gradient(135deg, #ffc107 0%, #ffd54f 100%); }
        .stats-approved { background: linear-gradient(135deg, #28a745 0%, #20c997 100%); }
        .stats-completed { background: linear-gradient(135deg, #17a2b8 0%, #5bc0de 100%); }
    ...
    <div class="row mb-4">
        ...
        <div class="col-md-3 col-sm-6">
            <div class="stats-card stats-payment">
                <h3 class="mb-2"><?php echo $status_counts['pending_payment']; ?></h3>
                <p class="mb-0"><i class="fas fa-clock mr-2"></i> Menunggu Bayar</p>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="stats-card stats-approved">
                <h3 class="mb-2"><?php echo $status_counts['approved']; ?></h3>
                <p class="mb-0"><i class="fas fa-lock mr-2"></i> Terkunci</p>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
        ...
        <div class="card-body">
            <div class="d-flex flex-wrap">
                ...
                <span class="filter-badge badge badge-success mr-2 mb-2 p-2" onclick="filterStatus('approved')">
                    <i class="fas fa-lock mr-1"></i> Terkunci (<?php echo $status_counts['approved']; ?>)
                </span>
                ...
                <span class="filter-badge badge badge-dark mr-2 mb-2 p-2" onclick="filterStatus('cancelled')">
                    <i class="fas fa-ban mr-1"></i> Dibatalkan (<?php echo $status_counts['cancelled']; ?>)
                </span>
            </div>
        </div>
    </div>
            ...
            <?php if(mysqli_num_rows($q) > 0): ?>
                <div id="bookingList">
                    <?php while($r = mysqli_fetch_assoc($q)): 
                        ...
                        // Cek apakah QRIS hampir expired (kurang dari 5 menit)
                        $qris_timer = '';
                        if ($r['status'] == 'pending_payment' && $r['qris_expiry_time'] && $r['qris_minutes_left'] > 0) {
                            if ($r['qris_minutes_left'] <= 5) {
                                $qris_timer = '<span class="timer-danger" title="Segera bayar!">
                                    <i class="fas fa-stopwatch mr-1"></i>' . $r['qris_minutes_left'] . 'm ' . ($r['qris_seconds_left'] % 60) . 's
                                </span>';
                            } elseif ($r['qris_minutes_left'] <= 10) {
                                $qris_timer = '<span class="timer-warning" title="Waktu hampir habis!">
                                    <i class="fas fa-clock mr-1"></i>' . $r['qris_minutes_left'] . 'm
                                </span>';
                            }
                        }
                        ...
                        // Cek apakah booking hampir expired (kurang dari 5 menit)
                        $show_urgent_payment = false;
                        if ($r['status'] == 'pending_payment' && $r['qris_minutes_left'] > 0 && $r['qris_minutes_left'] <= 5) {
                            $show_urgent_payment = true;
                        }
                    ?>
                    
                    <div class="booking-card" data-status="<?php echo $r['status']; ?>">
                        <?php if($show_urgent_payment): ?>
                        <div class="urgent-payment">
                            <i class="fas fa-exclamation-triangle mr-2"></i>
                            <strong>SEGERA BAYAR!</strong> Sisa waktu: <?php echo $r['qris_minutes_left']; ?> menit
                        </div>
                        <?php endif; ?>
                        ...
                        <div class="booking-body">
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <?php elseif($r['status'] == 'approved' && $r['payment_status'] == 'paid'): ?>
                                            ...
                                            <div class="alert alert-success mt-2 mb-0 p-2">
                                                <small><i class="fas fa-info-circle mr-1"></i> Booking sudah terkunci dan tidak dapat diubah</small>
                                            </div>
                            ...
                            <?php if($r['status'] == 'pending_payment' && $r['qris_expiry_time'] && $r['qris_minutes_left'] <= 0): ?>
                            ...
                            <?php if($r['status'] == 'pending_payment' && $r['qris_minutes_left'] > 0): ?>
                                <div class="alert alert-info mt-3 mb-0 p-2">
                                    <i class="fas fa-clock mr-2"></i>
                                    <strong>Sisa Waktu Pembayaran:</strong> 
                                    <?php echo $r['qris_minutes_left']; ?> menit 
                                    (<?php echo date('H:i', strtotime($r['qris_expiry_time'])); ?>)
                                    <br>
                                    <small>Booking akan otomatis dibatalkan jika tidak dibayar dalam 15 menit</small>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <small>
                        ...
                        <span class="badge status-pending-payment ml-2">Menunggu Bayar (15m)</span>
                        ...
                        <span class="badge badge-dark ml-1">Dibatalkan</span>
                    </small>
                </div>
                <div class="col-md-6 text-right">
                    <small>
                        ...
                        <?php 
                        // Hitung booking yang sedang menunggu pembayaran
                        $pending_payment_query = mysqli_query($conn, 
                            "SELECT COUNT(*) as count FROM bookings 
                             WHERE customer_id = $cid 
                             AND status = 'pending_payment' 
                             AND qris_expiry > NOW()");
                        $pending_payment_data = mysqli_fetch_assoc($pending_payment_query);
                        $pending_count = $pending_payment_data['count'] ?? 0;
                        if ($pending_count > 0): ?>
                        | <span class="text-warning">
                            <i class="fas fa-clock mr-1"></i>
                            <?php echo $pending_count; ?> booking menunggu pembayaran
                        </span>
                        <?php endif; ?>
                    </small>
                </div>
...
$(document).ready(function() {
    ...
    // Auto-refresh jika ada timer yang kurang dari 5 menit
    const timers = document.querySelectorAll('.timer-danger, .timer-warning');
    if (timers.length > 0) {
        setTimeout(function() {
            location.reload();
        }, 60000); // Reload setiap 1 menit jika ada timer aktif
    }
});
</script>
